...
import { SERVICES_DATA, PERSONAL_DATA, LUCENA_WEBSITE_PRODUCT_DATA } from '../data/portfolioData';
import { ServiceItem } from '../types';

interface ServicesWikiProps {
  onNavigateToLucenaWebsite?: () => void;
}

export const ServicesWiki: React.FC<ServicesWikiProps> = ({ onNavigateToLucenaWebsite }) => {
  ...
  return (
    <section id="servicos" className="space-y-6 scroll-mt-20">
      ...
      {/* Featured Model: Site para Advocacia (Lucena & Associados Advogados) */}
      <div 
        id="site-advocacia-lucena"
        className="p-4 sm:p-5 rounded-md border border-[#0969da]/30 bg-gradient-to-r from-[#f6f8fa] via-[#ffffff] to-[#ddf4ff]/30 shadow-2xs space-y-3"
      >
        <div className="flex flex-wrap items-center justify-between gap-2">
          <div className="flex items-center gap-2">
            <span className="px-2 py-0.5 rounded text-[10px] font-bold uppercase tracking-wider bg-[#0969da] text-white font-mono">
          ...
          <span className="text-[11px] font-medium text-[#1a7f37] bg-[#dafbe1] px-2 py-0.5 rounded border border-[#4ac26b]/30">
            Conforme OAB Provimento 205/2021
          </span>
        </div>

        <p className="text-xs text-[#57606a] leading-relaxed wiki-serif">
          Estrutura pronta e personalizada para bancas e advogados autônomos. Alto padrão visual, captação via WhatsApp, páginas de especialidades e SEO otimizado para o Google.
        </p>

        <div className="flex flex-wrap items-center gap-2.5 pt-1">
          {onNavigateToLucenaWebsite && (
            <button
              type="button"
              onClick={onNavigateToLucenaWebsite}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md bg-[#0969da] hover:bg-[#085cc0] text-white text-xs font-semibold shadow-2xs transition-colors cursor-pointer"
            >
              <span>Ver Página Completa do Modelo</span>
              <ArrowRight className="w-3.5 h-3.5" />
            </button>
          )}
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-[#d0d7de] bg-[#ffffff] hover:bg-[#f6f8fa] text-[#1f2328] text-xs font-semibold shadow-2xs transition-colors"
          >
            <ExternalLink className="w-3 h-3 text-[#0969da]" />
          ...
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md border border-[#d0d7de] bg-[#f6f8fa] hover:bg-[#eaeef2] text-[#1a7f37] text-xs font-semibold shadow-2xs transition-colors"
          >
            <MessageCircle className="w-3 h-3" />
